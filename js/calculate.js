...
function calcCost(formData, jqForm, options) {	
           ...
           if(isNumeric(form.scholarships.value)  && form.scholarships.value.length >0) 
               ...
               if(isNumeric(form.veterans.value)  && form.veterans.value.length >0) 
               aid_total = (parseInt(form.veterans.value) +  aid_total) *1; 
                        
           if ( form.scholarship.value  == "hope" &&  form.residency.value == "is") {
                hope_fee = form.enrollment.value *  1064; 
                
                hope_coverd = currency(  ( tuition_cost  - (tuition_cost * .8443) )  ) +" NOT covered by HOPE.";
                tuition_paid =  currency((tuition_cost * .8443)) +" covered by HOPE.    "+ hope_coverd; 
                aid_total = (tuition_cost * .8443) + aid_total; 
           }
...
$(document).ready(function(){ 
      ...
      $('.boxyinfo').click(function() {
            ...
            var info_url="wp-content/plugins/undergrad-calculator/info.html #"+this.id;
...
function toggle(lmnt) {	
		...
		if (  do_toggle ) {		
         ...
         if (lmnt=="school"){
                  ...
                  var chosenstatus=entrystatus.options[entrystatus.selectedIndex].value;
                  if (chosenstatus=="freshman" || chosenstatus=="transfer_oos"){
                           document.getElementById("yearyouentered").selectedIndex=0;
                           document.getElementById("semesterdiv").style.display="none";
                  }
         }
